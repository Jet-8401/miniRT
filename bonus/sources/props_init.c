...
int	triangle_init(t_scene *scene, char **args)
{
	...
	if (ft_strlen2(args) != 5)
		return (parser_error("Bad arguments number triangle"), -1);
	if (set_vector3d(&triangle->v0, args[1]) == -1)
		return (parser_error("Bad coordinate v0 triangle"), -1);
	if (set_vector3d(&triangle->v1, args[2]) == -1)
		return (parser_error("Bad coordinate v1 triangle"), -1);
	if (set_vector3d(&triangle->v2, args[3]) == -1)
		return (parser_error("Bad coordinate v2 triangle"), -1);
	if (set_rgb(args[4], &triangle->color) == -1)
		return (parser_error("Bad rgb triangle"), -1);
