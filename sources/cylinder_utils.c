...
static void	check_cylinder_height(t_ray_view *ray, t_object *cy, double *t)
{
	t_vec3	q;
	t_vec3	temp;
	...
	vec3_subtract(&q, &cy->pos, &temp);
	if (vec3_dot(&cy->dir, &temp) < 0)
		*t = -1;
	vec3_subtract(&q, &cy->end_cap->pos, &temp);
	if (vec3_dot(&cy->dir, &temp) > 0)
		*t = -1;
}
