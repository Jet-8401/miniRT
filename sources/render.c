...
// entry point antho -> pixel_draw(scene, &render);
		...
		for (int x = 0; x < scene->display.width; x++) {
			...
			if (instersect_forms(scene, &ray, &hit))
			{
				scene->display.data[x + y * scene->display.width] =
					rgb_to_int(light_handler(scene, render, &hit),
						scene->display.big_endian);
			}
			else
...
{
    t_vec3		light_dir = {0, 0, 0};
    t_ray_view	ray;

    ft_memset(&ray, 0, sizeof(ray));
    vec3D_subtract(&scene->light->pos, &hit->hit, &ray.direction);
    render->light_distance = vec3_length(ray.direction);
    ray.origin = hit->hit;
...
t_rgb light_handler(t_scene *scene, t_render *render, t_hit *hit)
{
    ...
    if (vec3D_dot(&hit->norm, &render->prime_ray.direction) > 0)
    	vec3D_mult(&hit->norm, -1, &hit->norm);
    vec3D_subtract(&scene->light->pos, &hit->hit, &diff_pos);
    vec3D_normalize(&diff_pos); // maybe delete
